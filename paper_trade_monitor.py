...
def _canonical_ticker(ticker: str) -> str:
    """Normalize repository/exchange ticker aliases before market-data calls.

    In particular, strategy signals may use XAUT/USDT while the Binance
    TradFi perpetual market is XAUUSDT. Keeping this normalization here
    prevents a symbol-format mismatch from silently breaking monitoring.
    """
...
def resolve_open_trade(trade_row: dict, provider) -> dict:
    ...
    if df is None or df.empty:
        return {"status": "STILL_OPEN", "locked_level_r": 0.0,
    ...
    position, exit_index = resolve_position_over_bars(
        ...
        opposite_index=None,
        symbol=ticker,
        tp=tp,  # FIX: recorded TP is now actually enforced by the monitor.
    )
...
def _maybe_notify_trailing_level(trade_row: dict, locked_level_r: float):
    ...
    If price jumps from below 0.6R to 1.8R, notifications for 0.6R, 1.2R
    and 1.8R are sent in order. If Telegram fails part-way through, the
    last successfully persisted level is retried on the next run.
    """
    ...
    if target <= 0 or target <= last_notified:
        return

    next_level = math.floor((last_notified + TRAIL_STEP_R + 1e-9) / TRAIL_STEP_R) * TRAIL_STEP_R
    while next_level <= target + 1e-9:
        ...
        print(msg)
        next_level = round(next_level + TRAIL_STEP_R, 10)
...
def _format_close_message(trade_id, trade_row, exit_reason, realized_r,
                          realized_pnl, duration_wall_clock, exit_price=None):
...
def _close_and_notify(trade_row: dict, result: dict, execution_layer):
    ...
    if position.direction == "BUY":
        realized_pnl = position.exit_price - position.entry
    else:
        realized_pnl = position.entry - position.exit_price

    # If a bar both reaches a new trailing level and closes the trade,
    # notify the newly locked levels before the final close notification.
...
def _retry_unnotified_closes():
    ...
    for row in load_unnotified_closed_trades():
        ...
        if exit_reason == "ENTRY_VALIDITY_SKIPPED":
            ...
            msg = _format_close_message(
                trade_id, row, exit_reason, realized_r, realized_pnl,
                duration, row.get("exit_price"),
            )
